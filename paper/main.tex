...
over two orders of magnitude lower per-frame latency than a 50-step
UNet diffusion decoder (1.98\,ms vs.\ 1{,}026\,ms on A100), while
maintaining stable multi-step rollouts (1.39$\times$ error ratio,
defined as MSE at frame~8 / MSE at frame~1).
A spectral frequency-matching loss recovers 56\% of suppressed latent
variance (ratio deviation $0.260{\to}0.113$), reducing perceptual
distance from LPIPS~0.223 to 0.177---within 3.5\% of the copy
baseline's 0.171, despite copy using ground-truth VAE encodings.
...
\item We provide ablation experiments showing that the residual
  ...
  with).
\item We demonstrate over two orders of magnitude lower per-frame
  latency compared to a 50-step UNet diffusion baseline
  (1.98\,ms at batch size~1 vs.\ 1{,}026\,ms at batch size~32).
...
The full system maintains a stability ratio of $1.39\times$
(MSE at frame~8 / MSE at frame~1), outperforming the copy baseline
($1.67\times$).
Removing slot cross-attention causes catastrophic drift
($3.16\times$), confirming that object-level structure is essential
for temporally coherent multi-step prediction even though it slightly
hurts single-step MSE (Table~\ref{tab:ablation}).
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    (schematic illustration based on rollout experiments over 50
    validation videos; stability ratios computed from actual data,
    curve shapes are approximate).
...
The spectral loss variant ($\beta{=}0.01$) closes 88\% of the LPIPS
gap to the copy baseline (which uses ground-truth VAE encodings),
reducing LPIPS from $0.223$ to $0.177$ compared to copy's $0.171$.
Over 8-frame rollouts, accumulated error increases LPIPS to $0.308$,
roughly $2\times$ the copy rollout ($0.154$), reflecting compounding
off-manifold drift.
...
Table~\ref{tab:speed} compares per-frame inference latency on an
A100 GPU.
Our hybrid system produces a frame prediction in $1.98$\,ms (batch
size~1), compared to $1{,}026$\,ms for a matched UNet with 50
denoising steps (batch size~32).
Note that the different batch sizes make a direct throughput comparison
difficult; the latency gap would narrow under equal batching.
Nevertheless, the architectural difference---single-pass cross-attention
vs.\ iterative denoising---yields a fundamental efficiency advantage.

\begin{table}[t]
\centering
\caption{%
  \textbf{Per-frame inference latency} on A100.
  UNet times measured with a matched UNet2DModel (75.4M params,
  matched in spatial resolution and channel count) at batch
  size 32; our method timed at batch size 1.
  \textbf{Caveat:} The different batch sizes mean the per-sample
  latency gap would narrow under equal batching conditions.
  We report this comparison to illustrate the architectural
  efficiency difference, not as a controlled throughput benchmark.
}
\label{tab:speed}
\begin{tabular}{@{}lccc@{}}
\toprule
Method & Batch Size & Latency (ms) & Ratio \\
\midrule
UNet 50-step & 32 & 1{,}026 & 1.0$\times$ \\
UNet 20-step & 32 & 410 & 2.5$\times$ \\
\textbf{Hybrid (Ours)} & 1 & \textbf{1.98} & \textbf{518}$\times$$^\dagger$ \\
\bottomrule
\multicolumn{4}{@{}l}{\scriptsize $^\dagger$Not directly comparable due to different batch sizes.}
\end{tabular}
\end{table}
...
We omit FID and FVD because our validation set (10 videos $\times$ 8
frames) provides insufficient samples for reliable distributional
metrics; we plan to report these on the full UCF-101 test set.
